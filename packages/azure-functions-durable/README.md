...
- **`client.getStatus()` keeps the v3 shape** — it returns a non-optional `DurableOrchestrationStatus`
  ...
  **`client.startNew()` supports the `version` and `orchestrationIdReusePolicy` options.** The reuse
  policy supports status-based duplicate errors and atomic replacement; the current shared protocol
  does not expose an atomic no-op/`IGNORE` action.
