...
/**
 * E2E tests for client history queries and worker history streaming in DTS.
 *
 ...
 *   - DTS_HISTORY_STREAMING_ROUNDS: Bounded streaming workload (default: 1, maximum: 8)
 */
...
describe("Worker history streaming negotiation", () => {
  // Keep the bounded scenario in this CI-selected file so Node 22/24 exercise it automatically.
  it("hydrates service-selected history and completes a replayed durable outcome", async () => {
    ...
    // Keep each payload below 1 MiB while accumulating a larger orchestration history.
    ...
    try {
      ...
      if (streamedWorkItems.length === 0) {
        throw new Error(
          `The service did not request history streaming within the bounded ${rounds}-activity workload.`,
        );
      }
