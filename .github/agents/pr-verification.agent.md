...
- Do NOT delete the verification sample — it has been pushed to the
  `verification/pr-<number>` branch along with the e2e tests.
...
- **Verification artifacts only:** This agent creates verification samples in
  `examples/verification/` and adds e2e tests in `test/e2e-azuremanaged/`. It does
  NOT modify any existing SDK source files in the repository.
...
- E2e tests are added to `test/e2e-azuremanaged/` covering the fix scenario
- Evidence is posted to the correct GitHub issue
- Verification samples and e2e tests are pushed to `verification/pr-<N>` branches
