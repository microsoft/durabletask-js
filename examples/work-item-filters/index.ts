 ...
 * - Disabling filters via useWorkItemFilters(null)
 *
...
async function runWithDisabledFilters() {
  console.log("\n=== Scenario 3: Disabled Filters (null) ===");
  console.log("The worker sends no filters to the sidecar, so it receives ALL work items.");
  console.log("Use this when you want a single worker to handle everything.\n");
  ...
  await worker.start();
  console.log("Worker started with filters DISABLED (receives all work items)");

  const id = await client.scheduleNewOrchestration(greetingOrchestrator, "No-Filters");
  console.log(`Scheduled orchestration: ${id}`);
