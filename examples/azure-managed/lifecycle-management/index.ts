...
//   2. Terminate (recursive) — cancel parent + child orchestrations
//   3. Suspend / Resume — pause and unpause an orchestration
//   4. Continue-as-new — restart an orchestration with new input
//   5. Restart — re-run a completed orchestration
//   6. Purge — delete orchestration history
//   7. Tags — attach metadata to orchestrations
...
/** Parent orchestration with a child (for recursive terminate demo). */
const parentOrchestrator: TOrchestrator = async function* (ctx: OrchestrationContext): any {
  const childResult: string = yield ctx.callSubOrchestrator(longRunning);
...
(async () => {
  ...
  try {
    ...
    // --- 2. Terminate (recursive) ---
    console.log("\n=== 2. Terminate (recursive — parent + child) ===");
    const parentId = await client.scheduleNewOrchestration(parentOrchestrator);
    await new Promise((r) => setTimeout(r, 3000)); // let child start
    await client.terminateOrchestration(parentId, terminateOptions({ output: "Force stop", recursive: true }));
    const parentState = await client.waitForOrchestrationCompletion(parentId, true, 15);
    console.log(`Parent status: ${OrchestrationStatus[parentState!.runtimeStatus]}`);
